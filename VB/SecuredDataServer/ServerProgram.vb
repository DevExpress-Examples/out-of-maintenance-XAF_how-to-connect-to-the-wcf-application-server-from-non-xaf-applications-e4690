	...
	Friend NotInheritable Class Program
		...
		Shared Sub Main()
			Try
				...
				XafTypesInfo.Instance.RegisterEntity(GetType(Stuff))
				...
				uow.UpdateSchema(GetType(Stuff), GetType(SecuritySystemUser), GetType(SecuritySystemRole))
		...
		Private Shared Sub CreateObjects(ByVal uow As UnitOfWork)
			Dim obj1 As New Stuff(uow)
			obj1.Name = "Available object"
			Dim obj2 As New Stuff(uow)
			obj2.Name = "Protected object"
		End Sub
		...
		Private Shared Sub CreateUsers(ByVal uow As UnitOfWork)
			Dim admin As SecuritySystemUser = uow.FindObject(Of SecuritySystemUser)(New BinaryOperator("UserName", "Admin"))
				...
				userRole.Name = "Users"
				userRole.AddObjectAccessPermission(Of Stuff)("[Name] == 'Available object'", SecurityOperations.Read)
